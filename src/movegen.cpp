...
// Is the king belonging to `board.color` currently attacked, given the
// position exactly as represented in `board` (no move applied)?
// Note: getKingMovesBB/getKnightMovesBB/etc. mask out squares occupied by
// board.color's own pieces, which is exactly what we want here since we
// probe attacks *from* the king's square outward using the same move
// tables (symmetric attack property of these piece types).
bool isInCheck(const ChessBoard& board) {
    uint64_t kingBB = board.getPieceBB(Piece::KING, board.color);
    if (kingBB == EMPTY_BB)
        return false; // defensive: no king on board (shouldn't happen in real play)

    Square kingSq(static_cast<uint8_t>(lsbBitscan(kingBB)));
    Color opp = ~board.color;

    // Pawn check
    uint64_t oppPawns = board.getPieceBB(Piece::PAWN, opp);
    if ((PAWN_ATTACKS[colorIdx(board.color)][kingSq.index] & oppPawns) != 0ULL)
        return true;

    // Knight check
    uint64_t oppKnights = board.getPieceBB(Piece::KNIGHT, opp);
    if ((getKnightMovesBB(kingSq, board) & oppKnights) != 0ULL)
        return true;

    // King adjacency (illegal in real chess, but kept for safety/symmetry)
    uint64_t oppKing = board.getPieceBB(Piece::KING, opp);
    if ((getKingMovesBB(kingSq, board) & oppKing) != 0ULL)
        return true;

    // Bishop/Queen diagonal
    uint64_t oppBishops = board.getPieceBB(Piece::BISHOP, opp);
    uint64_t oppQueens  = board.getPieceBB(Piece::QUEEN,  opp);
    if ((getBishopMovesBB(kingSq, board) & (oppBishops | oppQueens)) != 0ULL)
        return true;

    // Rook/Queen straight
    uint64_t oppRooks = board.getPieceBB(Piece::ROOK, opp);
    if ((getRookMovesBB(kingSq, board) & (oppRooks | oppQueens)) != 0ULL)
        return true;
